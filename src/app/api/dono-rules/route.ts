...
import { DonoRuleConfigSchema } from "@/lib/schemas/workspace";
...
function mergeConfig(
  current: z.infer<typeof DonoRuleConfigSchema>,
  next?: unknown,
  ...
  } as z.infer<typeof DonoRuleConfigSchema>;

  if (typeof enabledOverride === "boolean") {
    merged.enabled = enabledOverride;
  }

  return DonoRuleConfigSchema.parse(merged);
}
...
export async function POST(req: Request) {
  ...
  if (entitlements?.dono_rules_limit != null) {
    ...
    if (typeof count === "number" && count >= entitlements.dono_rules_limit) {
      return NextResponse.json(
        { error: "Dono rule limit reached for your plan." },
        { status: 403 },
      );
    }
  }

  const config = DonoRuleConfigSchema.parse(parsed.data.config ?? {});
...
export async function PATCH(req: Request) {
  ...
  if (fetchError || !existing) {
    return NextResponse.json({ error: "Dono rule not found." }, { status: 404 });
  }

  const currentConfig = DonoRuleConfigSchema.parse(existing.config ?? {});
